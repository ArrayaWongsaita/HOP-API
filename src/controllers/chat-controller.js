...
//====================== SEN MESSAGE =======================
chatController.senMessage = async (io,socket,chatId, senderId, content,senderRole)=>{
  try {
    console.log(`Received message from ${senderRole} id ${senderId} in chat ${chatId}: ${content}`);
    const message = await chatService.createMessageByChatIdSenderIdAndContent(chatId, senderId, content,senderRole)
